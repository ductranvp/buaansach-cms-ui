  ...
  methods: {
    subscribeTopics() {
      ...
      try {
        ...
        this.subscription = this.wsStompClient.subscribe(
          WebsocketEndpoints.TOPIC_POS_PREFIX + storeGuid,
        ...
        if (this.retry < this.maxRetry) {
          ...
          MessageUtils.error(
            "Không thể đăng ký nhận thông báo gọi món. Hãy thử tải lại trang."
          );
        }
      }
    },
    ...
    playAudio(elementId) {
      if (
        localStorage.getItem(StorageKey.localStorageKeys.MUTE_SOUND) !== "yes"
      ) {
    ...
    onMessageReceived(payload) {
      ...
      switch (data.message) {
        case WebsocketMessages.GUEST_CALL_WAITER:
          this.$store.commit(
            "posMachine/ADD_CALL_WAITER_NOTIFICATION",
            notification
          );
        ...
        case WebsocketMessages.GUEST_STORE_PAY_REQUEST:
          this.$store.commit(
            "posMachine/ADD_PAY_REQUEST_NOTIFICATION",
            notification
          );
        ...
        case WebsocketMessages.GUEST_UPDATE_PHONE:
          ...
          if (this.selectedSeat.guid === seatData.guid)
            MessageUtils.info(
              notification.title + " đã cập nhật số điện thoại."
            );
          break;
      }
      ...
      function reloadSeatIfActive(vm) {
        ...
        vm.$store
          .dispatch("posMachine/getSeatOrderInfo", seatData.guid)
          .then(() => {
            vm.scrollToEnd();
          });
      }
