<template>
  <el-dropdown :hide-on-click="false" trigger="click">
    <el-tooltip content="Yêu cầu thanh toán">
    ...
    <el-dropdown-menu class="padding-0 notification" slot="dropdown" v-loading="isLoading">
      <el-row class="bg-success text-light top-toolbar padding-5" type="flex" align="middle">
        <el-col class="padding-right-10">
          <span
            class="padding-right-10 text-bold">Thanh toán tại bàn ({{today | moment("DD/MM/YYYY")}})</span>
        </el-col>
      ...
      <template v-else>
        <el-dropdown-item class="padding-0" v-for="(noti) in listNotification" :key="noti.guid">
          <store-pay-request-item :notification="noti"/>
        </el-dropdown-item>
      </template>
    ...
    <hidden-store-pay-request-dialog ref="hiddenNotificationDialog"/>
  </el-dropdown>
</template>
...
<script>
  ...
  import StorePayRequestItem from "@/views/private/pos-machine/header/store-pay-request/StorePayRequestItem";
  import HiddenStorePayRequestDialog
    from "@/views/private/pos-machine/header/store-pay-request/HiddenStorePayRequestDialog";

  export default {
    name: "StorePayRequestNotification",
    components: {HiddenStorePayRequestDialog, StorePayRequestItem},
    computed: {
      ...mapState({
        listNotification: state => state.posMachine.storePayRequestNotifications.filter(item => !item.hidden),
        listUnseen: state => state.posMachine.storePayRequestNotifications.filter(item => !item.hidden && item.storePayRequestStatus === 'UNSEEN'),
        listSeen: state => state.posMachine.storePayRequestNotifications.filter(item => !item.hidden && item.storePayRequestStatus === 'SEEN'),
      })
    },
    data() {
      let today = new Date();
    ...
    methods: {
      async reloadNotification() {
        try {
          ...
          await this.$store.dispatch("posMachine/getStorePayRequestNotifications", {
