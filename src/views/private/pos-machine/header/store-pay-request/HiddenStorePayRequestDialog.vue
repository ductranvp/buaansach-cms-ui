    ...
    <el-main>
      ...
      <template v-else>
        <el-row class="notification-item" v-for="item in listInvisibleNotification" :key="item.guid">
          <store-pay-request-item :notification="item" :show-username="true" :selectable="false"/>
        </el-row>
      </template>
    </el-main>
    <div slot="footer">
...
<script>
  ...
  export default {
    name: "HiddenStorePayRequestDialog",
    components: {StorePayRequestItem},
    computed: {
      ...mapState({
        listInvisibleNotification: state => state.posMachine.storePayRequestNotifications.filter(item => item.hidden),
      })
    },
    data() {
      return {
