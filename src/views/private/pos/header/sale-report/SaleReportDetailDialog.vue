          ...
          <el-dropdown trigger="click" :hide-on-click="false">
            ...
            <el-dropdown-menu slot="dropdown">
              <el-dropdown-item v-for="key in Object.keys(columns)" :key="key">
                <el-checkbox v-model="columns[key].display">{{
                  columns[key].label
                }}</el-checkbox>
              </el-dropdown-item>
            </el-dropdown-menu>
    ...
    <el-main>
      <raw-data-table :data="reportData" show-index>
        <template v-for="key in Object.keys(columns)">
          ...
          >
            <template slot-scope="{ row }">
              <el-tag
                v-if="
                  columns[key].type === 'enum' && columns[key].enum[row[key]]
                "
              ...
              <div v-else-if="columns[key].type === 'time'">
                <span v-if="row[key]">{{
                  row[key] | moment("HH:mm:ss - DD/MM/YYYY")
                }}</span>
              </div>
              <div v-else-if="columns[key].type === 'seat'">
                <span
                  >{{ allSeatsObject[row[key]].seatName }} -
                  {{ allSeatsObject[row[key]].areaName }}</span
                >
              </div>
              <div v-else-if="columns[key].type === 'history'"></div>
        ...
        <el-table-column
          label="Lịch sử"
          v-if="
            ['STORE_MANAGER', 'STORE_OWNER'].includes(this.currentStoreUserRole)
          "
        >
          <template slot-scope="{ row }">
            <el-button
...
export default {
  ...
  data() {
    return {
      ...
      columns: {
        ...
        orderCustomerPhone: { label: "SĐT khách", display: false },
  ...
  methods: {
    ...
    onSearch() {
      if (this.searchKey) {
        this.reportData = this.originalReportData.filter(
          item =>
            item.orderCode.includes(this.searchKey) ||
            (item.orderCustomerPhone &&
              item.orderCustomerPhone.includes(this.searchKey))
        );
      } else {
