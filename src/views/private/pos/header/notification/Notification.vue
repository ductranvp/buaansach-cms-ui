<template>
  <el-dropdown :hide-on-click="false" trigger="click" :placement="placement">
    <!--Icon-->
    ...
    <!--Main-->
    <el-dropdown-menu
      class="padding-0 notification"
      slot="dropdown"
      v-loading="isLoading"
    >
      <el-row
        class="bg-success text-light top-toolbar padding-5"
        type="flex"
        align="middle"
      >
        <el-col class="padding-right-10">
          <span class="padding-right-10 text-bold"
            >{{ notificationTitle }} ({{ today | moment("DD/MM/YYYY") }})</span
          >
        </el-col>
        <el-button
          size="mini"
      ...
      <el-row
        class="bg-success text-light bottom-toolbar"
        type="flex"
        align="middle"
      >
        <el-col>
          <el-button
            @click="hideSeenNotification"
            type="info"
            size="small"
...
export default {
  ...
  computed: {
    ...mapState({
      ...
      listSeen(state) {
        return this.getListNotificationByType(state).filter(
          item =>
            item.storeNotificationStatus === StoreNotificationStatus.value.SEEN
        );
      },
      listUnseen(state) {
        return this.getListNotificationByType(state).filter(
          item =>
            item.storeNotificationStatus ===
            StoreNotificationStatus.value.UNSEEN
        );
      },
