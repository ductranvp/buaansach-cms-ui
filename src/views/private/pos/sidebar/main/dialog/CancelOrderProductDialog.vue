    ...
    <el-form ref="dialogForm" :model="form" :rules="formRules">
      <el-form-item prop="selectReason">
        ...
        <el-radio-group v-model="form.selectReason" @change="changeReason">
          <el-radio
            v-for="key in Object.keys(instantReasons)"
            :label="instantReasons[key]"
            :key="key"
          >
      ...
      <el-form-item
        prop="inputReason"
        v-if="form.selectReason === instantReasons.OTHER"
      >
        <input-label label="Nhập lý do khác tại đây" required />
        <el-input
          :disabled="form.selectReason !== instantReasons.OTHER"
          type="textarea"
          rows="3"
          v-model="form.inputReason"
        ></el-input>
      </el-form-item>
    </el-form>
    <div slot="footer">
  ...
  data() {
    return {
      ...
      instantReasons: {
        ...
        CANCEL_ORDER_PRODUCT_5: "Nhân viên gọi món",
        OTHER: "Khác"
      },
      form: {
        selectReason: null,
