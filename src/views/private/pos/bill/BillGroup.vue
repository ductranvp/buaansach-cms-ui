  ...
  export default {
    ...
    methods: {
      ...
      getBillMeta(order) {
        ...
        switch (order.seat.areaType) {
          case AreaType.value.IN_STORE:
            meta += '<tr><td>Vị trí: ' + order.seat.seatName + ' - ' + order.seat.areaName + '</td></tr>';
            break;
      ...
      getBillProduct(order) {
        ...
        listOrderProduct.forEach((op) => {
          if (listDistinctProduct[op.productGuid]) {
            listDistinctProduct[op.productGuid].orderProductQuantity += op.orderProductQuantity;
          } else {
        ...
        Object.keys(listDistinctProduct).forEach(key => {
          tbody += '<tr><td colspan=\'4\'>' + listDistinctProduct[key].productName + '</td></tr>';
          tbody += '<tr>';
          tbody += '<td>' + listDistinctProduct[key].productCode + '</td>';
          tbody += '<td>' + this.formatPrice(listDistinctProduct[key].orderProductPrice) + '</td>';
          tbody += '<td>' + listDistinctProduct[key].orderProductQuantity + '</td>';
          tbody += '<td class=\'text-right\'>' + this.formatPrice(this.getOrderProductPrice(listDistinctProduct[key])) +
            '</td>';
          tbody += '</tr>';
        });
        tbody += '</tbody>';
      ...
      getBillSummary(order) {
        ...
        tableContent += '<tr><th>DÙNG ĐIỂM</th><td class=\'text-right\'>' + order.orderPointValue + '</td></tr>';
        tableContent += '</table>';
        tableContent += '<table>';
      ...
      getTotalBillSummary() {
        ...
        tableContent += '<tr><th>TIỀN KHÁCH ĐƯA</th><td class=\'text-right\'>' + this.formatPrice(this.customerPay) +
          '</td></tr>';
        tableContent += '<tr><th>TIỀN TRẢ LẠI</th><td class=\'text-right\'>' +
          this.formatPrice(this.customerPay - this.getListOrderPayAmount(this.listOrder)) + '</td></tr>';
        tableContent += '</table>';
        return tableContent;
      },
      getBillHead() {
