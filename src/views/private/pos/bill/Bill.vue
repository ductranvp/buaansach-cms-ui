<template>
  <div style="width: 0; height: 0; visibility: hidden">
    <iframe id="bill_frame" width="0" height="0" frameborder="0"> </iframe>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    getBillProduct() {
      const orderProduct = this.savedOrderProduct.filter(
        od => od.orderProductStatus === OrderProductStatus.value.SERVED
      );
      let distinctProduct = {};

      orderProduct.forEach(op => {
        if (distinctProduct[op.productGuid]) {
          distinctProduct[op.productGuid].orderProductQuantity +=
      ...
      Object.keys(distinctProduct).forEach(key => {
        tbody +=
          "<tr><td colspan='4'>" +
          distinctProduct[key].productName +
          "</td></tr>";
        tbody += "<tr>";
        tbody += "<td>" + distinctProduct[key].productCode + "</td>";
        tbody +=
          "<td>" +
          this.formatPrice(distinctProduct[key].orderProductPrice) +
          "</td>";
        tbody += "<td>" + distinctProduct[key].orderProductQuantity + "</td>";
        tbody +=
          "<td class='text-right'>" +
          this.formatPrice(this.getOrderProductPrice(distinctProduct[key])) +
          "</td>";
        tbody += "</tr>";
      });
      tbody += "</tbody>";
    ...
    getBillSummary(customerPay) {
      ...
      tableContent +=
        ...
        this.currentOrder.orderPointValue +
        "</td></tr>";
      tableContent += "</table>";
      ...
      tableContent +=
        ...
        this.formatPrice(this.payAmount) +
        "</b></td></tr>";
      tableContent +=
        "<tr><th>TIỀN KHÁCH ĐƯA</th><td class='text-right'>" +
        this.formatPrice(customerPay) +
        "</td></tr>";
      tableContent +=
        "<tr><th>TIỀN TRẢ LẠI</th><td class='text-right'>" +
        this.formatPrice(customerPay - this.payAmount) +
        "</td></tr>";
      tableContent += "</table>";
      return tableContent;
    },
