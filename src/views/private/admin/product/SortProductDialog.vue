  ...
  >
    <el-container v-loading="isLoading" class="full-width">
      <el-header height="auto">
        <el-row
          class="padding-10 bg-primary text-light text-bold text-large"
          type="flex"
          align="middle"
        >
          <el-col :span="2">
      ...
      <el-main ref="sortableProduct">
        ...
        >
          ...
          <el-col :span="4">
            <div>
              <el-tag type="success" v-if="product.productActivated"
                >Đã bật</el-tag
              >
              <el-tag type="danger" v-else>Đã tắt</el-tag>
            </div>
          </el-col>
          <el-col :span="5">
            <div>
              <el-tag
                type="primary"
                v-if="product.productType === productTypes.MAIN_PRODUCT"
                >Sản phẩm chính</el-tag
              >
              <el-tag type="warning" v-else>Sản phẩm phụ</el-tag>
            </div>
          </el-col>
          <el-col :span="5">
...
export default {
  ...
  methods: {
    ...
    async show() {
      ...
      this.$nextTick(() => {
        DragNDropUtils.setSortable(
          this,
          this.$refs.sortableProduct.$el,
          this.handleSort
        );
      });
    },
    async handleSort(evt) {
      ...
      let update = DragNDropUtils.getNewPos(
        current,
        before,
        after,
        "productPosition"
      ...
      try {
        ...
        NotificationUtils.error(
          "Lỗi sắp xếp vị trí sản phẩm, vui lòng thử lại sau!"
        );
        this.getProduct();
      }
    },
